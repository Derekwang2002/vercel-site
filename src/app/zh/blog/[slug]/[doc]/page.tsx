...
export async function generateStaticParams(): Promise<Array<{ doc: string; slug: string }>> {
  return (await getAllPostSeriesDocuments("zh")).map((document) => ({
    doc: document.slug,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return getPostSeriesDocumentMetadata({ docSlug: doc, locale: "zh", seriesSlug: slug });
}

export default async function ChinesePostSeriesDocumentRoute({ params }: Props) {
  const { doc, slug } = await params;
  return <PostSeriesDocumentPage docSlug={doc} locale="zh" seriesSlug={slug} />;
}
