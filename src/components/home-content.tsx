...
import { getFeaturedResources, getResourceTypeLabel, isExternalResourceHref } from "../../lib/resources";
...
export async function HomeContent({ locale }: { locale: "en" | "zh" }) {
  const resources = (await getFeaturedResources()).slice(0, 3).map((item) => localizeResource(item, locale));
    ...
    <main className={styles.home} lang={locale === "zh" ? "zh-CN" : "en"}>
      ...
      {resources.length > 0 ? (
        <section className={styles.pinnedSection} id="pinned" aria-labelledby="pinned-title">
          <div className={styles.pinnedHeader}><h2 id="pinned-title">{locale === "zh" ? "置顶" : "Pinned"}</h2><Link href={`${prefix}/hub/all`}>{locale === "zh" ? "查看全部" : "View all"}</Link></div>
          <ul className={styles.pinnedList}>{resources.map((resource) => (
            <li className={styles.pinnedItem} key={`${resource.type}-${resource.href}`}>
              <div className={styles.pinnedItemHeader}><PinnedResourceLink locale={locale} resource={resource} /><span className={styles.pinnedMeta}>{getResourceTypeLabel(resource.type)}</span></div>
...
function PinnedResourceLink({ locale, resource }: { locale: "en" | "zh"; resource: Resource }) {
  if (isExternalResourceHref(resource.href)) return <a className={styles.pinnedLink} href={resource.href} rel="noreferrer" target="_blank">{resource.title}</a>;
  const href = resource.type === "skill" && locale === "zh" ? `/zh${resource.href}` : resource.href;
  return <Link className={styles.pinnedLink} href={href}>{resource.title}</Link>;
}
