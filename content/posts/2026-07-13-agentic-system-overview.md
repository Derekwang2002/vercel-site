...
> 如果希望从零学习 CALL-E Agentic Goal 架构，请从[《CALL-E Agentic Goal 架构从零理解：从聊天目标到真实电话执行》](/zh/blog/calle-agentic-goal)开始。
...
这四个对象只是仓库地图上的坐标。它们的状态机、事件、租约、游标、Workspace 与源码调用关系，请继续阅读本系列第 0 篇：[《CALL-E Agentic Goal 架构从零理解》](/zh/blog/calle-agentic-goal)。本文不再重复展开。
...
先读[《CALL-E Agentic Goal 架构从零理解》](/zh/blog/calle-agentic-goal)，再按文章中的源码顺序进入 `services/seleven-mcp/src/calle/agentic/`。这样能先建立 Goal、Iteration、RunSpec、Run、Event 和 Report 的心智模型，再看具体函数。
