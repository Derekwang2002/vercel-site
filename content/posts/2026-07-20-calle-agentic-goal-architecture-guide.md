...
- **第 1 篇（计划中）**：追踪 `commit_goal`，理解 Goal、Event 和 Dispatch 如何写入数据库。
- **第 2 篇（计划中）**：逐行阅读 `GoalIterationRunner`，理解 Lease、Cursor、事务与恢复。
- **第 3 篇（计划中）**：追踪 `RunSpec → Run → VoiceRunExecutor` 的真实电话执行链路。
